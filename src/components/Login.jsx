...
export default function Login({ setToken }) {
    ...
    const handleLogin = async () => {
        if (!judgeID || !password) {
            setError('Please enter both Judge ID and password');
            return;
        }
        ...
        try {
            setLoading(true);
            setError('');
            const { data } = await axios.post('https://hackathon-judging-backend.vercel.app/api/auth/login', { judgeID, password });
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 px-4">
            <div className="max-w-md w-full bg-white rounded-xl shadow-lg overflow-hidden">
                <div className="bg-gradient-to-r from-indigo-600 to-purple-600 py-6">
                    <h2 className="text-2xl font-bold text-white text-center">Judge Login</h2>
                </div>
